...
int mqtt3_handle_connect(struct mosquitto_db *db, struct mosquitto *context)
{
    ...
    _mosquitto_log_printf(NULL, MOSQ_LOG_INFO, "In handle connect");
	...
	if(will){
		...
		if(_mosquitto_read_string(&context->in_packet, &will_topic)){
			rc = 1;
            _mosquitto_log_printf(NULL, MOSQ_LOG_ERR, "we r here.");
			goto handle_connect_error;
		}
	...
	if(will_struct){
        _mosquitto_log_printf(NULL, MOSQ_LOG_NOTICE,"i ahve reached will");
	...
	if(db->config->connection_messages == true){
		if(context->is_bridge){
			...
			if(context->username){
				...
				_mosquitto_log_printf(NULL, MOSQ_LOG_NOTICE, "New client connected from %s as %s (c%d, k%d) with topic %s.", context->address, client_id, clean_session, context->keepalive, will_topic);
			}
		}
	}
...
int mqtt3_handle_subscribe(struct mosquitto_db *db, struct mosquitto *context)
{
	...
	while(context->in_packet.pos < context->in_packet.remaining_length){
        ...
        _mosquitto_log_printf(NULL, MOSQ_LOG_INFO, "Received SUBSCRIBE from %s", sub);
