...
int mosquitto_main_loop(struct mosquitto_db *db, mosq_sock_t *listensock, int listensock_count, int listener_max, int *pfds)
{
		...
		if(fdcount == -1){
			...
			for(i=0; i<listensock_count; i++){
				if(pollfds[i].revents & (POLLIN | POLLPRI)){
					...
					while(mqtt3_socket_accept(db, listensock[i],pfds) != -1){
					}
				}
			}
		}
